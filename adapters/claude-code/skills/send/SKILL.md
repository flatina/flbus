...
- Backticks/`$(...)` are shell-expanded in **any** inline arg (`--to`/`--summary`/`--subject` too — an expanded `` `x` `` can silently blank a name) — avoid them there. For the body use `--body-stdin` with a single-quoted heredoc (bash `<<'E'…E`) or here-string (PowerShell `@'…'@`), or `--body-file <.tmp/file>`
